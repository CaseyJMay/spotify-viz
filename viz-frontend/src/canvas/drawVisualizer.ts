...
function drawFooter(
  ...
  // Screen-derived metadata deliberately has no progress estimate. A negative
  // value suppresses the bar instead of showing a misleading empty track.
  if (song.progress >= 0) {
    const trackHeight = 2;
    const trackY = progressBarY + (PROGRESS_BAR_HEIGHT - trackHeight) / 2;
